...
ggplot(data=flights)+
  geom_bar(mapping=aes(x=dep_time,fill=hour))
...
```{r}
a <- filter(flights,month %in% c(7,8),day==1,!is.na('Na'))
ggplot(data=a)+
...
b <- arrange(a,desc(arr_delay))
select(flights,flight,carrier,arr_delay,dep_delay)
```
...
b <- mutate(a,
       Tiemporecuperado=dep_delay-arr_delay)
group_by(b,carrier)
